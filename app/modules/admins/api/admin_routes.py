...
async def create_system_user(
	...
	current_user: Annotated[dict[str, Any], Depends(require_permission(audience="system", permission=MANAGE_SYSTEM_USERS_PERMISSION))],
	service: Annotated[Any, Depends(get_admin_service)],
) -> SystemUserResponse:
	del current_user
...
async def get_system_user(
	user_id: UUID,
	current_user: Annotated[dict[str, Any], Depends(require_permission(audience="system", permission=VIEW_SYSTEM_USERS_PERMISSION))],
	service: Annotated[Any, Depends(get_admin_service)],
) -> SystemUserResponse:
	del current_user
...
async def list_roles(
	current_user: Annotated[dict[str, Any], Depends(require_permission(audience="system", permission=MANAGE_ROLES_PERMISSION))],
	service: Annotated[Any, Depends(get_admin_service)],
) -> RoleListResponse:
	del current_user
...
async def get_system_user_permissions(
	...
	current_user: Annotated[
		dict[str, Any],
		Depends(require_permission(audience="system", permission=VIEW_SYSTEM_PERMISSIONS_PERMISSION)),
	],
	service: Annotated[Any, Depends(get_admin_service)],
) -> SystemUserPermissionsResponse:
	del current_user
